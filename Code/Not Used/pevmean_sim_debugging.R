...
# This script is to test for a bug in the PEVmean simulation scenario
...
if (MSI) {
  ...
  setwd("C:/Users/Jeff/Google Drive/Barley Lab/Projects/Side Projects/Simulations/Hypred simulations/")
...
change = "PEVmean"
  ...
  #   set.results <- lapply(X = 1:length(iter.set), FUN = function(i) {
    ...
    for (i in 1:50) {
      ...
      TP.values <- measure.values(genome = hv.genome, gametes = input.gametes, h2 = h2, n.env = n.env, n.rep = n.rep, model = NULL)
      TP.phenos <- TP.values$pheno.mu
      ...
      for (breeding.cycle in 1:n.cycles) {
        ...
        system.time(genotyping.population.i <- make.population(genome = hv.genome, 
        ...
        # Measure the true genotypic values of the selection candidates
        candidate.geno.value <- measure.values(genome = hv.genome,
        ...
        pred.validation.i <- validate.predictions(predicted.GEBVs = candidate.i.GEBV,
                                                  observed.values = candidate.geno.value$geno.values,
                                                  boot.reps = 1000)
        ...
        parent.gametes <- subset.gametes(gametes = genotyping.population.i,
        ...
        if (! change == "no.change") {
          ...
          optimization.candidates <- setdiff(row.names(candidate.geno.value$geno.values), TP.addition.base$lines.sel)
          ...
          TP.addition.gametes <- subset.gametes(gametes = genotyping.population.i,
          ...
          TP.addition.values <- measure.values(genome = hv.genome,
                                               gametes = TP.addition.gametes,
                                               h2 = h2,
                                               n.env = n.env,
                                               n.rep = n.rep)
          # Separate the phenotypes
          TP.addtion.phenos <- TP.addition.values$pheno.mu
        ...
        simulation.results[[cycle.name]] <- list(candidate.genos = candidate.i.genos,
                                                 geno.summary.stats = list(pairwise.div = candidate.i.genos.pairwise.div,
                                                 ...
                                                 prediction.results = list(GEBV = candidate.i.prediction$GEBV,
                                                                           PEV = candidate.i.prediction$solve.out$PEVuhat,
                                                                           parameters = candidate.i.prediction$parameters),
                                                 candidate.genotype.values = candidate.geno.value,
